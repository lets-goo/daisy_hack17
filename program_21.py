...
def miniMAX(node, maximizingPlayer, we):
	...
	if node.isLeaf() or (isBigBoardWon("11"+node.board_data) != 0):
...
def free_move(board_data, we):
	...
	# 'we' is a charactor of either '1' or '2'
	PLAYER=int(board_data[0])
	nextsquare=int(board_data)
	squares = []
	for i in range(2,83): 
		squares.append(square(i-2,int(board_data[i-2])))
	validMoves=findValidMoves(squares,nextsquare)

# PLAYER=int('11'+root.children[0].board_data)
# nextsquare=int('11'+root.children[0].board_data)
# squares = []
# for i in range(2,83): 
#	squares.append(square(i-2,int(('11' +root.children[0].board_data)[i-2])))
# validMoves=findValidMoves(squares,nextsquare)
...
def board_modification(data, move, mover):
	...
	new_board = data[0:move] +  str(mover) + data[move+1:]
	return new_board	
...
def initialize_tree(root, move, board_data, depth, depth_counter, mover, player):
	...
	if (isSmallBoardWon(board_data, int(move))!=0):
		all_possible_mv_sc = free_move(str(player)+str(move)+str(board_data), mover)
...
def get_move(timeout,data):
	...
	if (nextsquare == "9" or data[2:83] == "0"*81):
		...
		root = initialize_tree(root, data[1], data[2:83], 5, 0, PLAYER, PLAYER)
		best_score = miniMAX(root, True, PLAYER)
		for i in range(9):
			child = root.get_children(i)
			if(child != None):
				if child.score == best_score:
					for i in range(9):
						state_parent = root.board_data[9*i:9*i+9]
						state_child = child.board_data[9*i:9*i+9]
						print(i)
						if state_parent != state_child:
							for j in range(9):
								if state_parent[j] != state_child[j]:
									return i*9 + j
